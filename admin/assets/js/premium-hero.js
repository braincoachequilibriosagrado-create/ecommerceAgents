(function (global) {
  ...
  var FRAG = [
    ...
    'varying vec2 vUv;',
    'const vec3 cNavy = vec3(0.039, 0.086, 0.157);',
    'const vec3 cBlue = vec3(0.102, 0.420, 1.0);',
    'const vec3 cBlueHi = vec3(0.48, 0.70, 1.0);',
    'const vec3 cWhite = vec3(0.94, 0.97, 1.0);',
    'void main() {',
    '  vec2 uv = vUv;',
    ...
    '  vec3 col = cNavy * 0.85;',
    '  float spokes = abs(fract(ang / 6.2831853 * 12.0 + t * 0.05) - 0.5);',
    '  float spoke = exp(-spokes * spokes * 920.0) * smoothstep(0.95, 0.06, rad);',
    '  float rings = abs(fract(rad * 4.4 - t * 0.14) - 0.5);',
    '  float ring = exp(-rings * rings * 480.0) * 0.62;',
    '  vec2 h = p * 6.0;',
    '  h.x += t * 0.09;',
    '  float hx = abs(fract(h.x) - 0.5);',
    '  float hy = abs(fract(h.y + h.x * 0.577) - 0.5);',
    '  float grid = exp(-min(hx, hy) * min(hx, hy) * 320.0) * 0.42;',
    '  float pulse = 0.55 + 0.45 * sin(rad * 9.0 - t * 3.6);',
    '  float nodes = exp(-pow(fract(rad * 3.2 - t * 0.45) - 0.5, 2.0) * 90.0) * spoke;',
    '  col += cBlue * (spoke * 0.95 + ring * 0.50 + grid * 0.45);',
    '  col += cBlueHi * ring * 0.28;',
    '  col += cWhite * nodes * 0.55 * pulse;',
    '  col *= 1.0 - 0.32 * smoothstep(0.25, 1.25, rad);',
